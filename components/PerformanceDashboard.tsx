    ...
    <div className="w-full space-y-8 pb-10">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          ...
          <div key={i} className="bg-[#F9FAFB] p-5 rounded-xl border border-slate-200 transition-all duration-200 flex flex-col justify-center hover:bg-white hover:shadow-sm">
            <p className="text-[#718096] text-[14px] font-medium mb-1 uppercase tracking-wider">{stat.label}</p>
            <p className="text-3xl font-bold text-[#2D3748]">{stat.val}</p>
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="space-y-4">
          <h3 className="text-xl font-bold text-slate-800 px-1">Topic Mastery</h3>
          <div className="bg-[#F9FAFB] p-5 rounded-xl border border-slate-200 transition-all duration-200 hover:bg-white hover:shadow-sm">
        ...
        {/* Line Chart */}
        <div className="space-y-4">
          <h3 className="text-xl font-bold text-slate-800 px-1">7-Day Accuracy Trend</h3>
          <div className="bg-[#F9FAFB] p-5 rounded-xl border border-slate-200 transition-all duration-200 hover:bg-white hover:shadow-sm">
            <div className="h-72 w-full">
              {dailyData.length > 0 ? (
                <ResponsiveContainer width="100%" height="100%">
      ...
      <div className="space-y-4">
        <div className="px-1">
          <h3 className="text-xl font-bold text-slate-800">Action Plan: Weak Topics</h3>
          <p className="text-[14px] text-[#718096] font-normal mt-1">Focus your studying on these high-priority areas</p>
        </div>
        <div className="bg-[#F9FAFB] rounded-xl border border-slate-200 overflow-hidden transition-all duration-200 hover:bg-white hover:shadow-sm">
          {sortedWeakTopics.length > 0 ? (
                  ...
                  <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div>
                      <div className="flex items-center gap-3 mb-2">
                        <h4 className="font-bold text-[#2D3748]">{topic.topic}</h4>
                        <span className="text-slate-400 text-sm">&bull;</span>
                        <span className="text-slate-600 text-sm font-medium">{topic.subtopic}</span>
                      </div>
                      <p className="text-sm text-slate-500 max-w-2xl leading-relaxed bg-blue-50/30 p-3 rounded-lg border border-blue-100/50">
            ...
            <div className="p-10 text-center">
              ...
              <h4 className="text-lg font-bold text-[#2D3748] mb-1">Excellent Work!</h4>
              <p className="text-[#718096] text-sm">No weak topics identified yet. Keep practicing!</p>
            </div>
          )}
        </div>
      </div>
