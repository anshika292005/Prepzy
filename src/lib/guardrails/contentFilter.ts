...
export const filterMCQOutput = async (
  mcqs: MCQQuestion[],
  topic: string
): Promise<MCQQuestion[]> => {
  ...
  return mcqs.filter((q) => {
    ...
    if (!q.explanation || q.explanation.length < 100) return false;

    // 3. Valid correct key
    if (!allowedOptions.includes(q.correct)) return false;
...
export const sanitizeExplanation = (text: string): string => {
  let clean = text;
